...
- **홈 보기 토글**(`layouts/index.html` + 인라인 JS): 🗂 **프로젝트별**(폴더=섹션명별 그룹) / ☰ **리스트**(전체 시간순). 버튼을 눌러 **그 자리서 전환**, 선택은 `localStorage` 기억. 기본 = 프로젝트별(리스트는 후순위, 버튼도 프로젝트별이 앞). 두 뷰 데이터를 한 페이지에 담고 JS로 show/hide(페이지 이동 없음).
- **프로젝트 배지**: 리스트 카드마다 프로젝트(폴더)명 배지. 클릭 → `/posts/<repo>/`(그 프로젝트 글만). 제목이 날짜뿐(`YYYY-MM-DD 회고`)이라 **같은 날짜 글의 구분자는 배지**다(제목에 프로젝트를 넣지 않는다). **프로젝트별 고유색** — 배지에 `data-proj="{{ .File.ContentBaseName }}"`(폴더 슬러그)를 달고 `custom.css`의 `.proj-badge[data-proj="..."]`로 색 매핑(흰 글자 솔리드, 라이트/다크 공통; 미정의 프로젝트는 폴백 회색). 색 추가는 CSS 한 줄. 상세 글 상단 배지(`_partials/breadcrumbs.html`)도 같은 색 — 단 거긴 `.CurrentSection.File`이 nil일 수 있어(About 등 루트 섹션) `{{ if .File }}`로 가드 필수.
